...
<template>
    <div class="task-entry flex-row">
        <input class="task-checkbox" type="checkbox" :checked="sessionData.cachedDays[date]?.getProgress(taskType, data.id)" :id="data.id" @change="(e) => handleTaskRecordChange(props.gameName,props.taskType,props.date, data, e.target.checked ? 1 : 0)"/>
        <p>{{ $t(`${data.id}`) }}</p>
